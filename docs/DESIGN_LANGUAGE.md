...
| 9 | **Auth (sign in / enrollment request)** | Light | **Encode the real enrollment model in the UI:** anonymous enrollment request → admin approves → account auto-created + temp password emailed. First-time users need a request path, not just a sign-in box. **Step 1 (login + enroll pages) done — see below; step 2 (authenticated shell/nav) and step 3 (mobile nav + touch-target sizing) still open.** |
...
**Enrollment-model note (product gap already identified):** nothing in the current UI explains that students *request* enrollment and get approved. Surfaces 1, 3, and 9 must make this path obvious to a first-time visitor. **Surface 1 (Landing) is done** — its "How it works" section (`id="how-it-works"`, the hero's secondary CTA target) spells out the real flow as four steps: browse cohorts → request enrollment (no account needed) → we review & approve → account auto-created & access. **Surface 9's step 1 (below) closes its share of this note** — surface 3 remains open.
...
**Status:** steps 1 and 2 are done — see `IMPLEMENTATION_ROADMAP.md`'s "Design System — Foundation" entry. Step 3 is underway: surfaces #1 (**Landing**), #2 (**Catalog**), #3 (**Course detail**), #4 (**Student dashboard**), #5 (**Course content / sessions**), #6 (**Assignments**), #7 (**Gradebook**), and #8 (**Certificates / verify**) are migrated — see `ARCHITECTURE.md` §6's migration notes for the nav mechanism used and the known limitations of each. Dashboard is the first surface built on the light lane (§2.3) — see the accent-on-background note there. Landing was built in two parts (hero + nav + motion foundation, then the content sections below it) — both parts are recorded together in `ARCHITECTURE.md` §6's landing note. **Surface #9 (Auth) is partially migrated** — its step 1 (the login and enrollment-request pages) is done; see `ARCHITECTURE.md` §6's surface #9 note for what steps 2–3 still cover. #10 Admin console still uses its pre-design-system styling.

**Certificates/verify (surface #8) shipped as a restyle, not new functionality** — both pages (`my-certificates`, `verify`) were already fully wired to real endpoints before this pass; only the visual layer changed. See `ARCHITECTURE.md` §6's certificates note for the tier/verify-status mapping file and the print-layout addition.

**Landing shipped "featured programs," not a distinct "certificate/verify strip"** the way §4 #1's row lists it. The Programs section (real courses from `GET /catalog/courses`, reusing `CourseCard`) covers "featured cohorts"; certificate verification is reachable from the footer nav (reusing the existing `certificates.verifyButton` link) rather than getting its own trust band. No certificate data exists to make a dedicated strip meaningful yet (a verify-by-code form belongs on `/verify` itself, §4 #8, not duplicated on the landing) — folding it into the footer was the honest call rather than building a decorative section around a link. Revisit if a real "N certificates issued" or similar earned stat becomes available.
