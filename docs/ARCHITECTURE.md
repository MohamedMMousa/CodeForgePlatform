...
| Gradebook | ✅ built (Phase 3) | Per-student attendance rate + best assessment score/pass + assignment final score, for both the instructor's course roster and a student's own course view. Admin cross-course analytics stays out of scope, reserved for Phase 4. `AssignmentGradeDto` also carries a computed `Passed` (surface #7 prep, `GradebookCalculator.BuildAssignmentGrades` calling the existing `AssignmentGradingCalculator.ComputePassed`) — same null discipline as `AssessmentGradeDto.Passed` (null when ungraded or the assignment has no `PassScore` threshold), so both roles' gradebooks badge assignment pass/fail consistently with assessments. Compute-don't-store, no new column. |
